import { LessThanEqualToValidator } from '../../../src/validators/number';
import { expect } from 'chai';

describe('number/less-than-equal-to', () => {
  it('should return false if the input is undefined', async () => {
    const validator = new LessThanEqualToValidator(1);
    const valid = await validator.isValid(undefined);
    expect(valid).to.be.false;
  });

  it('should return true if the number is less than the threshold', async () => {
    const validator = new LessThanEqualToValidator(10);
  ...
  it('should return true when the value equals the threshold', async () => {
    const validator = new LessThanEqualToValidator(10);
